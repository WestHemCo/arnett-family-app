...
export default function AddItem({ onSave, onCancel, defaultLocation = 'pantry' }) {
  ...
  async function startScan() {
    ...
    try {
      ...
      await html5QrCode.start(
        ...
        async (decodedText) => {
          await stopScanner()
          await handleUPCFound(decodedText)
        },
      ...
      if (err.message && err.message.includes('permission')) {
        setUpcStatus('camera_denied')
      } else {
        setUpcStatus('camera_error')
      }
    }
  }
  ...
  const MSG = {
    ...
    camera_denied:{ text:'Camera access denied — check your browser settings', color:'#DC2626' },
    camera_error: { text:'Camera error — try typing the UPC manually', color:'#DC2626' },
  }
    ...
    <div style={s.overlay}>
      <div style={s.modal}>
        ...
        <div style={s.upcRow}>
          ...
          <button style={s.scanBtn} onClick={scanning ? stopScanner : startScan}>
            {scanning ? 'Stop' : 'Scan'}
          </button>
        </div>

        {/* Scanner container — html5-qrcode renders into this */}
        {scanning && (
          <div style={s.scannerWrap}>
            <div id="scanner-container" style={s.scannerContainer}/>
            <p style={s.scanHint}>Point camera at a barcode</p>
          </div>
        )}
...
const s = {
  ...
  scannerWrap:      { margin:'12px 24px 0', borderRadius:'12px', overflow:'hidden',
                      background:'#000', padding:'8px' },
  scannerContainer: { width:'100%', minHeight:'200px' },
  scanHint:         { color:'#FFFFFF', fontSize:'13px', textAlign:'center',
